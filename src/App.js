...
fetch(`https://fakestoreapi.com/products?page=${page}&limit=10${category}`)
.then(res=>res.json())
.then((json)=>{
  setCollections(json)
}).catch((err)=>{
  ...
  return (
    <div className="App">
      ...
      <div className="content">
        {
         isLoading? <h2>Идет загрузка..</h2>: collections.filter((obj)=>obj.title.toLowerCase().includes(searchValue.toLowerCase())
         ).map((obj, index)=>(
